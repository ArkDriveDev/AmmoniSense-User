...
import { showToast as emitAppToast } from '../../utils/toast';
...
export const CreateSiteModal: React.FC<CreateSiteModalProps> = ({ isOpen, onClose, onSiteCreated, editSite }) => {
  ...
  const handleCreateSite = async () => {
    ...
    if (editSite) {
      try {
        ...
        if (!synced) {
          ...
          emitAppToast({ message: `Site "${form.site_name}" updated locally & queued for sync.`, color: 'warning' });
        } else {
          emitAppToast({ message: `Site "${form.site_name}" updated successfully!`, color: 'success' });
        }
        ...
        setToastMsg(errMsg);
        emitAppToast({ message: errMsg, color: 'danger' });
        setShowToast(true);
      } finally {
        setLoading(false);
      }
    ...
    try {
      ...
      emitAppToast({ message: successMsg, color: 'success' });
      setShowToast(true);
      ...
      emitAppToast({ message: offlineMsg, color: 'warning' });
      setShowToast(true);
